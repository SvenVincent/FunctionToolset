...
# change list:
#     1. docker install from alicloud mirror
...
if [ "${USE_MOBY}" = "true" ]; then
    ...
    curl -fsSL https://mirrors.aliyun.com/docker-ce/linux/${ID}/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg
    echo "deb [arch=amd64 signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://mirrors.aliyun.com/docker-ce/linux/${ID} ${VERSION_CODENAME} stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
...
#        curl -fsSL "https://github.com/docker/compose/releases/download/${compose_v1_version}/docker-compose-Linux-x86_64" -o /usr/local/bin/docker-compose
        ...
        curl -H 'X-JFrog-Art-Api:AKCp8k8iXoC12jWSf9givBbNhMGzPwDLKzXzQTaxSvzju8YWCQC7bgd6Jf3VyqFRsjF8FHiUA' "https://artifact.inceptio.tech/artifactory/testing/docker-compose-Linux-x86_64" -o /usr/local/bin/docker-compose
